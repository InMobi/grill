 ...
 */
public class GrillQueryDAO extends GrillServerDAO {
  ...
  /**
   * Method to create finished queries table, this is required for embedded
   * grill server. For production server we will not be creating tables as it
   * would be created upfront.
   */
  ...
  public FinishedGrillQuery getQuery(String handle) {
    ResultSetHandler<FinishedGrillQuery> rsh =
        new BeanHandler<FinishedGrillQuery>(FinishedGrillQuery.class);
